...
{
    public:
        Mrf24j(int pin_reset, int pin_chip_select, int pin_interrupt, int wakeup, SPIClass * spi);
        void reset(void);
        void init(void);
        ...
        void send16(word dest16, uint8_t* data, byte len);
        void send16(word dest16, word panid, word source, uint8_t* data, byte len);
        void send16(word dest16, char* data) ;

        // void send_packet(uint8_t *data, uint8_t len, uint address);
