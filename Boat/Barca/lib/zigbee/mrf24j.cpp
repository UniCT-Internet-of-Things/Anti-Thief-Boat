 ...
 */
Mrf24j::Mrf24j(int pin_reset, int pin_chip_select, int pin_interrupt, int wakeup, SPIClass * spi) {
    ...
    pinMode(_pin_wakeup, OUTPUT);
    digitalWrite(_pin_wakeup, HIGH);
    
    //pinMode(_pin_int, INPUT);
}

void Mrf24j::reset(void) {
    //write_short(RSTPWR, 0x07);
...
byte Mrf24j::read_short(byte address) {
    ...
    _spi->transfer(address<<1 & 0b01111110);
    byte ret = _spi->transfer(0x00);
    digitalWrite(_pin_cs, HIGH);
    return ret;
}
...
byte Mrf24j::read_long(word address) {
    ...
    byte alow = address << 5;
    _spi->transfer(0x80 | ahigh);  // high bit for long
    _spi->transfer(alow);
    byte ret = _spi->transfer(0);
    digitalWrite(_pin_cs, HIGH);
    return ret;
}
...
void Mrf24j::write_short(byte address, byte data) {
    ...
    _spi->transfer((address<<1 & 0b01111110) | 0x01);
    _spi->transfer(data);
    digitalWrite(_pin_cs, HIGH);
}
...
void Mrf24j::write_long(word address, byte data) {
    ...
    byte alow = address << 5;
    _spi->transfer(0x80 | ahigh);  // high bit for long
    _spi->transfer(alow | 0x10);  // last bit for write
    _spi->transfer(data);
    digitalWrite(_pin_cs, HIGH);
}
 ...
 * @param data
 */
void Mrf24j::send16(word dest16, uint8_t* data, byte len) {
    ...
    /*
        righe 144-145 sono invertite in questa funzione
        per permettere set corretto del PAN id lato robot
        -------------------------------------------------
        da risolvere successivamente lato robot del parsing
        del PAN id
    */
...
void Mrf24j::send16(word dest16, word panid, word source, uint8_t* data, byte len) {
    // byte len = strlen(data); // get the length of the char* array
    int i = 0;
    write_long(i++, bytes_MHR); // header length
    // +ignoreBytes is because some module seems to ignore 2 bytes after the header?!.
...
// void Mrf24j::send_packet(uint8_t *data, uint8_t len, uint address)
// {
//     static uint8_t seq_num = 0;
//     uint8_t i = 0x00;
//     uint16_t pan_id = get_pan();
//     uint16_t short_addr = address16_read();

//     // Header
//     write_long(i++, 9);       // header length
//     write_long(i++, 9 + len); // frame length

//     // data frame type, intra-PAN, ACK req., short address for dest. and src.
//     write_long(i++, 0x61);      // first  byte of frame control field (FCF)
//     write_long(i++, 0x88);      // second byte of frame control field (FCF)
//     write_long(i++, seq_num++); // sequence number

//     write_long(i++, pan_id);         // low-byte  of the destination PAN ID
//     write_long(i++, pan_id >> 8);    // high-byte of the destination PAN ID
//     write_long(i++, address & 0xff); // low-byte  of destination address
//     write_long(i++, address >> 8);   // high-byte of destination address

//     // note: src PAN ID is not required, because intra-PAN bit in FCF is set.

//     write_long(i++, short_addr);      // low-byte  of src short address
//     write_long(i++, short_addr >> 8); // high-byte of src short address
...
//     write_short(MRF_TXNCON, _reg.value);
...
//     _reg.value = read_short(MRF_TXSTAT);
//     if (_reg.txstat.TXNSTAT == 1)
//     {
//         _send_failed_count++;
//         Serial.println(F(">>> Packet sending error"));
//     }
//     _retransmissions += _reg.txstat.TXNRETRY;
// }
...
void Mrf24j::init(void) {
    ...
    delay(5); // delay at least 192usec
}
...
void Mrf24j::interrupt_handler(void) {
    ...
    if (last_interrupt & MRF_I_RXIF) {
        ...
        rx_info.panid = rx_buf[4] << 8 | rx_buf[3];
        rx_info.dest_addr = rx_buf[6] << 8 | rx_buf[5];
        rx_info.src_addr = rx_buf[8] << 8 | rx_buf[7];
...
void Mrf24j::enable_turbo(){
    write_short(MRF_BBREG0,0x01);
}

void Mrf24j::disable_turbo(){
    
    write_short(MRF_BBREG0,0x00);
}