...
static unsigned char RFM_Read(unsigned char RFM_Address, SPIClass &spi)
{
...
********************************************************************************************/
static void RFM_change_SF_BW(unsigned char _SF, unsigned char _BW, SPIClass &spi)
{
	RFM_Write(RFM_REG_MODEM_CONFIG2, (_SF << 4) | 0b0100, spi); //SFx CRC On
	RFM_Write(RFM_REG_MODEM_CONFIG1,(_BW << 4) | 0x02, spi); //x kHz 4/5 coding rate explicit header mode

  #if defined(EU_868) || defined(IN_865) || defined(AS_923) || defined(AS_923_2)
  if(_SF>10)
    RFM_Write(RFM_REG_MODEM_CONFIG3, 0b1100, spi); //Low datarate optimization on AGC auto on 
  else
    RFM_Write(RFM_REG_MODEM_CONFIG3, 0b0100, spi); //Mobile node, low datarate optimization on AGC acorging to register LnaGain  
  #else
...
*****************************************************************************************
*/
static void RFM_Change_Datarate(unsigned char Datarate, SPIClass &spi)
{
#if defined(US_915)
  switch (Datarate) {
  case 0x00:  // SF10BW125
  ...
  switch (Datarate) {
  case 0x00:  // SF12BW125
    RFM_change_SF_BW(12,0x07, spi);
    break;
  case 0x01:  // SF11BW125
    RFM_change_SF_BW(11,0x07, spi);
    break;
  case 0x02:  // SF10BW125
    RFM_change_SF_BW(10,0x07, spi);
    break;
  case 0x03:  // SF9BW125
    RFM_change_SF_BW(9,0x07, spi);
    break;
  case 0x04:  // SF8BW125
    RFM_change_SF_BW(8,0x07, spi);
    break;
  case 0x05:  // SF7BW125
    RFM_change_SF_BW(7,0x07, spi);
    break;
  case 0x06:  // SF7BW250
    RFM_change_SF_BW(7,0x08, spi);
    break;
  }
#endif
}
...
*****************************************************************************************
*/
static void RFM_Change_Channel(unsigned char Channel, SPIClass &spi)
{
  ...
  if (Channel <= 0x08) 
    for(unsigned char i = 0 ; i < 3 ; ++i)
      RFM_Write(RFM_REG_FR_MSB + i, pgm_read_byte(&(LoRa_Frequency[Channel][i])), spi);  
#elif defined(IN_865)
  if( Channel <= 0x03)
    for(unsigned char i = 0 ; i < 3 ; ++i)
      RFM_Write(RFM_REG_FR_MSB + i, pgm_read_byte(&(LoRa_Frequency[Channel][i]))); 
...
*****************************************************************************************
*/
bool RFM_Init(SPIClass &spi)
{
  
  uint8_t ver = RFM_Read(0x42, spi);
  if(ver!=18){
    return 0;
  }
  //Switch RFM to sleep
  //DON'T USE Switch mode function
  RFM_Write(RFM_REG_OP_MODE, RFM_MODE_SLEEP, spi);
  ...
  //DON'T USE Switch mode function
  RFM_Write(RFM_REG_OP_MODE ,RFM_MODE_LORA, spi);
  //Switch RFM to standby
  RFM_Switch_Mode(RFM_MODE_STANDBY, spi);
  //Set channel to channel 0
  RFM_Change_Channel(CH0, spi);
  //Set default power to maximun on US915 TODO AS/AU/EU config
  //Set the default output pin as PA_BOOST
  RFM_Set_Tx_Power(20, PA_BOOST_PIN, spi);

  //Switch LNA boost on
  RFM_Write(RFM_REG_LNA,0x23, spi);

  //Set RFM To datarate 0 SF12 BW 125 kHz
  RFM_Change_Datarate(0x00, spi);

  //Rx Timeout set to 37 symbols
  RFM_Write(RFM_REG_SYM_TIMEOUT_LSB, 0x25, spi);
  //RFM_Write(RFM_REG_SYM_TIMEOUT_LSB, 0x05);
  
  //Preamble length set to 8 symbols
  //0x0008 + 4 = 12
  RFM_Write(RFM_REG_PREAMBLE_MSB,0x00, spi);
  RFM_Write(RFM_REG_PREAMBLE_LSB,0x08, spi);

  //Set LoRa sync word
  RFM_Write(RFM_REG_SYNC_WORD, 0x34, spi);

  //Set FIFO pointers
  //TX base address
  RFM_Write(0x0E,0x80, spi);
  //Rx base address
  RFM_Write(0x0F,0x00, spi);
  return 1;
}


void RFM_Set_Tx_Power(int level, int outputPin, SPIClass &spi)
{
  if (RFO_PIN == outputPin) {
    ...
    RFM_Write(RFM_REG_PA_CONFIG, 0x70 | level, spi);
  } else {
    // PA BOOST
    if (level > 17) {
      if (level > 20) {
      ...
      RFM_Write(RFM_REG_PA_DAC, 0x87, spi);
      RFM_Set_OCP(140, spi);
    } else {
      if (level < 2) {
        level = 2;
      ...
      RFM_Write(RFM_REG_PA_DAC, 0x84, spi);
      RFM_Set_OCP(100, spi);
    }

    RFM_Write(RFM_REG_PA_CONFIG, 0x80 | (level - 2), spi);  //PA Boost mask
  }
}

bool RFM_isRxDone(SPIClass &spi)
{
    return (RFM_Read(RFM_REG_IRQ_FLAGS & 0x7f, spi) & IRQ_RX_DONE_MASK) != 0;
}

void RFM_Set_OCP(uint8_t mA, SPIClass &spi)
{
  uint8_t ocpTrim = 27;
  ...
  RFM_Write(RFM_REG_OCP, 0x20 | (0x1F & ocpTrim), spi);
}
...
void RFM_Send_Package(sBuffer *RFM_Tx_Package, sSettings *LoRa_Settings, SPIClass &spi)
{
  unsigned char i;
  ...
  //Switch Datarate
  RFM_Change_Datarate(LoRa_Settings->Datarate_Tx, spi);

  //Switch Channel
  RFM_Change_Channel(LoRa_Settings->Channel_Tx, spi);

  //Switch DIO0 to TxDone
  if (RFM_pins.DIO0 != -1)
    RFM_Write(RFM_REG_DIO_MAPPING1, 0x40, spi);

  //Set IQ to normal values
  RFM_Write(RFM_REG_INVERT_IQ,0x27, spi);
  RFM_Write(RFM_REG_INVERT_IQ2,0x1D, spi);

  //Set payload length to the right length
  RFM_Write(RFM_REG_PAYLOAD_LENGTH,RFM_Tx_Package->Counter, spi);

  //Get location of Tx part of FiFo
  RFM_Tx_Location = RFM_Read(0x0E,spi);

  //Set SPI pointer to start of Tx part in FiFo
  RFM_Write(RFM_REG_FIFO_ADDR_PTR, RFM_Tx_Location, spi);

  //Write Payload to FiFo
  for (i = 0;i < RFM_Tx_Package->Counter; i++)
  {
    RFM_Write(RFM_REG_FIFO, RFM_Tx_Package->Data[i], spi);
  }

  //Switch RFM to Tx
  RFM_Write(RFM_REG_OP_MODE,0x83, spi);
  ...
  {
    while ((RFM_Read(RFM_REG_IRQ_FLAGS & 0x7f, spi) & IRQ_TX_DONE_MASK) == 0);
  }

  //Clear interrupt
  RFM_Write(RFM_REG_IRQ_FLAGS,0x08, spi);
}
...
message_t RFM_Single_Receive(sSettings *LoRa_Settings, SPIClass &spi)
{
  message_t Message_Status = NO_MESSAGE;
  
  //Change DIO 0 back to RxDone
  if (RFM_pins.DIO0 != -1)
    RFM_Write(RFM_REG_DIO_MAPPING1, 0x00, spi);

  //Invert IQ Back
  RFM_Write(RFM_REG_INVERT_IQ, 0x67, spi);
  RFM_Write(RFM_REG_INVERT_IQ2, 0x19, spi);

  //Change Datarate
  RFM_Change_Datarate(LoRa_Settings->Datarate_Rx, spi);

  //Change Channel
  RFM_Change_Channel(LoRa_Settings->Channel_Rx, spi);

  //Switch RFM to Single reception
  RFM_Switch_Mode(RFM_MODE_RXSINGLE, spi);
  ...
  {
    RegIrqFlags = RFM_Read(RFM_REG_IRQ_FLAGS & 0x7f, spi);
  ...
  if (isTimeout)
  {
    ...
    RFM_Write(RFM_REG_IRQ_FLAGS,0xE0, spi);
...
*****************************************************************************************
*/
void RFM_Continuous_Receive(sSettings *LoRa_Settings, SPIClass &spi)
{
...
#if defined(EU_868)
  RFM_Change_Datarate(SF12BW125, spi);
  RFM_Change_Channel(CHRX2, spi);
#elif defined(IN_865)
  ...
  //Serial.println("Cannel Rx "+String(LoRa_Settings->Channel_Rx));
	...
	RFM_Switch_Mode(RFM_MODE_RXCONT, spi);
}
...
message_t RFM_Get_Package(sBuffer *RFM_Rx_Package, SPIClass &spi)
{
  unsigned char i;
  ...
  RFM_Interrupts = RFM_Read(0x12, spi);
  ...
  RFM_Package_Location = RFM_Read(0x10, spi); /*Read start position of received package*/
  RFM_Rx_Package->Counter = RFM_Read(0x13, spi); /*Read length of received package*/

  RFM_Write(RFM_REG_FIFO_ADDR_PTR,RFM_Package_Location, spi); /*Set SPI pointer to start of package*/

  for (i = 0x00; i < RFM_Rx_Package->Counter; i++)
  {
    RFM_Rx_Package->Data[i] = RFM_Read(RFM_REG_FIFO, spi);
  }

  //Clear interrupt register
  RFM_Write(RFM_REG_IRQ_FLAGS,RFM_Interrupts, spi);
...
void RFM_Write(unsigned char RFM_Address, unsigned char RFM_Data, SPIClass &spi)
{
...
*****************************************************************************************
*/
void RFM_Switch_Mode(unsigned char Mode, SPIClass &spi)
{
    Mode = Mode | 0x80; //Set high bit for LoRa mode

    //Switch mode on RFM module
    RFM_Write(RFM_REG_OP_MODE,Mode, spi);
}
...
*****************************************************************************************
*/
unsigned char RFM_Get_Rssi(SPIClass &spi)
{
  return RFM_Read(RFM_REG_LAST_RSSI, spi);
}
