...
void LORA_Cycle(sBuffer *Data_Tx, sBuffer *Data_Rx, RFM_command_t *RFM_Command, sLoRa_Session *Session_Data,
 									sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message_Rx, sSettings *LoRa_Settings, msg_t *upMsg_Type, SPIClass &spi)
{
	...
	if(*RFM_Command == NEW_RFM_COMMAND){
		...
		//Lora send data & ack
		if(*upMsg_Type == MSG_UP) LORA_Send_Data(Data_Tx, Session_Data, LoRa_Settings, spi);
		else if(*upMsg_Type == MSG_ACK) LORA_Send_ACK(Data_Tx, Session_Data, LoRa_Settings, spi);
		...
		if(LoRa_Settings->Mote_Class == CLASS_C){
			...
			LoRa_Settings->Datarate_Rx = SF12BW500;   //set RX2 datarate 12
			#endif
			LORA_Receive_Data(Data_Rx, Session_Data, OTAA_Data, Message_Rx, LoRa_Settings, spi);  //BUG DETECT SENDED PACKET ALWAYS (IT DOES UPDATE)
		}
		...
		do{
			LORA_Receive_Data(Data_Rx, Session_Data, OTAA_Data, Message_Rx, LoRa_Settings, spi);
...
*****************************************************************************************
*/
void LORA_Send_Data(sBuffer *Data_Tx, sLoRa_Session *Session_Data, sSettings *LoRa_Settings, SPIClass& spi)
{
	//Define variables
	unsigned char i;
	...
	//Send Package
	RFM_Send_Package(&RFM_Package, LoRa_Settings, spi);
...
void LORA_Send_ACK(sBuffer *Data_Tx, sLoRa_Session *Session_Data, sSettings *LoRa_Settings, SPIClass &spi)
{
...
*****************************************************************************************
*/
void LORA_Receive_Data(sBuffer *Data_Rx, sLoRa_Session *Session_Data, sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message, sSettings *LoRa_Settings, SPIClass &spi)
{
	unsigned char i;
	...
	{
		Message_Status = RFM_Single_Receive(LoRa_Settings, spi);  
	}
	else
	{
	...
	{
		Message_Status = RFM_Get_Package(&RFM_Package, spi);

		//If mote class C switch RFM back to continuous receive
		if(LoRa_Settings->Mote_Class == CLASS_C)
		{
			//Switch RFM to Continuous Receive
			RFM_Continuous_Receive(LoRa_Settings, spi);
		}
	}
...
*****************************************************************************************
*/
void LoRa_Send_JoinReq(sLoRa_OTAA *OTAA_Data, sSettings *LoRa_Settings, SPIClass &spi)
{
	unsigned char i;
...
bool LORA_join_Accept(sBuffer *Data_Rx,sLoRa_Session *Session_Data, sLoRa_OTAA *OTAA_Data, sLoRa_Message *Message, sSettings *LoRa_Settings, SPIClass &spi)
{
